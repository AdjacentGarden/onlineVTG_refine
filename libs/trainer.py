...
class Trainer:
    ...
    def _microbatch_forward_backward(self, data_list, is_last=False):
        ...
        if is_last or not self.opt['_distributed']:
            fpn_logits, _, past_fpn_offsets, future_fpn_offsets, fpn_masks = \
        ...
        past_fpn_offsets = torch.cat(past_fpn_offsets, dim=2).flatten(0, 1)
        ...
        pred_past = past_fpn_offsets[pos_masks]
        pred_future = future_fpn_offsets[pos_masks]
        pos_gt_offsets = gt_offsets[pos_masks]
        gt_past = pos_gt_offsets[:, :1]
        gt_future = pos_gt_offsets[:, 1:]
        zero_offsets = torch.zeros_like(pred_past)
        past_pred_offsets = torch.cat(
            [pred_past, zero_offsets], dim=-1
        )
        future_interval_offsets = torch.cat(
            [zero_offsets, pred_future], dim=-1
        )
        past_gt_offsets = torch.cat(
            [gt_past, torch.zeros_like(gt_past)],
            dim=-1,
        )
        future_gt_offsets = torch.cat(
            [torch.zeros_like(gt_future), gt_future],
            dim=-1,
        )

        past_reg_loss = self._calc_iou_loss(
            pred_offsets=past_pred_offsets, gt_offsets=past_gt_offsets
        ) / self.loss_norm * get_world_size()
        future_reg_loss = self._calc_iou_loss(
            pred_offsets=future_interval_offsets, gt_offsets=future_gt_offsets
        ) / self.loss_norm * get_world_size()
        reg_loss = (
            self.past_reg_loss_weight * past_reg_loss
