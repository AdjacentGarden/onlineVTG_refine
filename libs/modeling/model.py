...
class OnVTG(nn.Module):
    def __init__(self, opt):
        ...
        self.past_reg_head = make_head(opt['past_reg_head'])
    ...
    def fuse_and_predict(self, fpn, fpn_masks, text, text_masks, text_size=None):
        ...
        past_fpn_offsets, _ = self.past_reg_head(fpn, fpn_masks)
        future_fpn_offsets, fpn_masks = self.future_reg_head(fpn, fpn_masks)

        fpn_offsets = tuple(
            torch.cat([p, f], dim=-1)
            for p, f in zip(past_fpn_offsets, future_fpn_offsets)
        )
